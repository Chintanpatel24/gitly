function generatePRCard({ username, pullRequests = [] }) {
  ...
  for (const repo of repos) {
    ...
    rows += `
      ...
      <text x="${PAD + 12}" y="${y + 23}" font-family="${FONT}" font-size="12" fill="${C.sub}">o</text>
    ...
    for (let i = 0; i < prs.length; i++) {
      ...
      rows += `<rect x="${PAD}" y="${y}" width="${W - PAD*2}" height="${PR_H}" fill="${C.headerBg}"/>`;
      rows += `<rect x="${PAD}" y="${y}" width="3" height="${PR_H}" fill="${sc}" opacity="0.7"/>`;
      rows += `<line x1="${PAD}" y1="${y}" x2="${W - PAD}" y2="${y}" stroke="${C.divider}" stroke-width="1"/>`;
  ...
  <g transform="translate(17,17) scale(1.5)" fill="${C.sub}">
  ...
  <text x="46" y="29" font-family="${FONT}" font-size="15" font-weight="700" fill="${C.title}">Pull Requests</text>
  <text x="46" y="48" font-family="${FONT}" font-size="12" fill="${C.sub}">@${xe(username)} - ${total} total</text>
